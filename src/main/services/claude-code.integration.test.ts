...
import { RefinedResumeSchema, GeneratedCoverLetterSchema } from '../../schemas/ai-output.schema'
...
function getTestDataPaths(): { resume: string; jobPosting: string } {
  ...
  for (const basePath of possibleBasePaths) {
    const resumePath = path.join(basePath, 'Auto Run Docs/Working/test-sample-resume.json')
    const jobPostingPath = path.join(basePath, 'Auto Run Docs/Working/test-job-posting.txt')
...
async function testZodValidationCatchesMalformed(): Promise<void> {
  ...
  try {
    ...
    if (error instanceof Error && error.message === 'Schema should have rejected malformed data') {
      throw error
    }
  ...
  try {
    ...
    if (error instanceof Error && error.message === 'Schema should have rejected invalid email') {
      throw error
    }
...
async function testSanitizationRemovesArtifacts(): Promise<void> {
  ...
  if (cleanedZeroWidth !== 'HelloWorld!') {
    throw new Error(`Zero-width chars not removed: "${cleanedZeroWidth}" (expected "HelloWorld!")`)
  }
  ...
  if (cleanedSpaces !== 'Hello World') {
    throw new Error(`Spaces not normalized: "${cleanedSpaces}" (expected "Hello World")`)
  }
  ...
  if (sanitizedObject.name !== 'Test User' || sanitizedObject.highlights[0]?.includes('**')) {
...
async function testResumeRefinement(): Promise<void> {
  ...
  console.log(`  Summary: ${validatedOutput.personalInfo.summary?.substring(0, 100)}...`)
...
async function testCoverLetterGeneration(): Promise<void> {
  ...
  const coverLetter = await processor.generateCoverLetter(validatedResume, jobPosting, {
    companyInfo: {
...
async function main(): Promise<void> {
  ...
  if (runLiveTests) {
    ...
    console.log(
      '       Example: npx ts-node --esm src/main/services/claude-code.integration.test.ts --live'
    )
  }
