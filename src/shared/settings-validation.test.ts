...
function assertErrorCode(
  actual: { code: SettingsValidationErrorCode } | null,
  expectedCode: SettingsValidationErrorCode,
  testName: string
): void {
...
testGroup('Hex Color Validation', () => {
  // Valid hex colors
  assertNull(validateHexColor('#000000', 'color'), 'Accepts valid hex color #000000')

  assertNull(validateHexColor('#ffffff', 'color'), 'Accepts valid hex color #ffffff (lowercase)')

  assertNull(validateHexColor('#FFFFFF', 'color'), 'Accepts valid hex color #FFFFFF (uppercase)')

  assertNull(validateHexColor('#1a2b3c', 'color'), 'Accepts valid hex color #1a2b3c (mixed)')
...
testGroup('Font Validation', () => {
  // Valid fonts
  assertNull(validateFont('Helvetica', 'font'), 'Accepts Helvetica font')

  assertNull(validateFont('Times-Roman', 'font'), 'Accepts Times-Roman font')

  assertNull(validateFont('Courier-Bold', 'font'), 'Accepts Courier-Bold font')
...
testGroup('Font Size Validation', () => {
  // Valid font sizes
  assertNull(validateFontSize(24, 'fontSize', 12, 36), 'Accepts font size within range')

  assertNull(validateFontSize(12, 'fontSize', 12, 36), 'Accepts font size at minimum')

  assertNull(validateFontSize(36, 'fontSize', 12, 36), 'Accepts font size at maximum')
...
testGroup('Spacing Validation', () => {
  // Valid spacing
  assertNull(validateSpacing(40, 'spacing', 20, 80), 'Accepts spacing within range')

  assertNull(validateSpacing(20, 'spacing', 20, 80), 'Accepts spacing at minimum')

  assertNull(validateSpacing(80, 'spacing', 20, 80), 'Accepts spacing at maximum')
...
testGroup('Resume Prompt Template Validation', () => {
  ...
  // Invalid values
  const invalidSummaryLength: ResumePromptTemplateSettings = {
    ...validTemplate,
    maxSummaryLength: 50,
  }
  ...
  const invalidHighlights: ResumePromptTemplateSettings = {
    ...validTemplate,
    maxHighlightsPerExperience: 15,
  }
  result = validateResumePromptTemplate(invalidHighlights)
  assertTrue(result.errors.length > 0, 'Rejects highlights above maximum')

  const longInstructions: ResumePromptTemplateSettings = {
    ...validTemplate,
    customInstructions: 'a'.repeat(2500),
  }
...
testGroup('Cover Letter Prompt Template Validation', () => {
  ...
  // Invalid values
  const invalidOpening: CoverLetterPromptTemplateSettings = {
    ...validTemplate,
    maxOpeningLength: 50,
  }
  ...
  const invalidParagraphs: CoverLetterPromptTemplateSettings = {
    ...validTemplate,
    maxBodyParagraphs: 10,
  }
...
testGroup('Errors to Field Map', () => {
  ...
  assertEqual(fieldMap['pdfTheme.colors.primary'], 'Invalid color format', 'Maps full field path')

  assertEqual(fieldMap['fileNamingPattern'], 'Pattern is empty', 'Maps simple field name')

  // Simplified key should also be present
  assertTrue('primary' in fieldMap, 'Simplified key is present')
})
