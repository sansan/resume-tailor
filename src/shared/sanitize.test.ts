...
testGroup('Zero-Width Character Removal', () => {
  ...
  assertEqual(removeZeroWidthChars('hello\u200Bworld'), 'helloworld', 'Removes zero-width space')

  // Zero Width Non-Joiner (U+200C)
  assertEqual(removeZeroWidthChars('test\u200Cvalue'), 'testvalue', 'Removes zero-width non-joiner')

  // Zero Width Joiner (U+200D)
  assertEqual(removeZeroWidthChars('test\u200Dvalue'), 'testvalue', 'Removes zero-width joiner')

  // Byte Order Mark / Zero Width No-Break Space (U+FEFF)
  assertEqual(removeZeroWidthChars('\uFEFFHello'), 'Hello', 'Removes BOM at start')
  ...
  // Word Joiner (U+2060)
  assertEqual(removeZeroWidthChars('word\u2060joiner'), 'wordjoiner', 'Removes word joiner')
})
...
testGroup('Invisible Control Character Removal', () => {
  // Null character
  assertEqual(removeInvisibleChars('hello\u0000world'), 'helloworld', 'Removes null character')

  // Bell character
  assertEqual(removeInvisibleChars('test\u0007value'), 'testvalue', 'Removes bell character')

  // Backspace
  assertEqual(removeInvisibleChars('test\u0008value'), 'testvalue', 'Removes backspace character')

  // Preserves tabs
  assertEqual(removeInvisibleChars('hello\tworld'), 'hello\tworld', 'Preserves tab characters')

  // Preserves newlines
  assertEqual(removeInvisibleChars('hello\nworld'), 'hello\nworld', 'Preserves newline characters')

  // Delete character (U+007F)
  assertEqual(removeInvisibleChars('test\u007Fvalue'), 'testvalue', 'Removes delete character')
})
...
testGroup('Space Normalization', () => {
  ...
  assertEqual(normalizeSpaces('hello world'), 'hello world', 'Regular spaces unchanged')
})
...
testGroup('Line Ending Normalization', () => {
  // CRLF to LF
  assertEqual(normalizeLineEndings('hello\r\nworld'), 'hello\nworld', 'Converts CRLF to LF')

  // Standalone CR to LF
  assertEqual(normalizeLineEndings('hello\rworld'), 'hello\nworld', 'Converts standalone CR to LF')

  // Multiple CRLFs
  assertEqual(normalizeLineEndings('a\r\nb\r\nc'), 'a\nb\nc', 'Converts multiple CRLFs to LFs')

  // Mixed line endings
  assertEqual(normalizeLineEndings('a\r\nb\rc\nd'), 'a\nb\nc\nd', 'Handles mixed line endings')
...
testGroup('Multiple Space Collapse', () => {
  // Two spaces
  assertEqual(collapseMultipleSpaces('hello  world'), 'hello world', 'Collapses two spaces to one')
  ...
  assertEqual(collapseMultipleSpaces('a  b   c    d'), 'a b c d', 'Collapses multiple occurrences')

  // Single spaces unchanged
  assertEqual(collapseMultipleSpaces('hello world'), 'hello world', 'Single spaces unchanged')

  // Leading spaces preserved (for indentation)
  assertEqual(collapseMultipleSpaces('  hello'), '  hello', 'Leading spaces preserved')
})
...
testGroup('Blank Line Normalization', () => {
  ...
  assertEqual(normalizeBlankLines('a\n\n\nb'), 'a\n\nb', 'Reduces three newlines to two')

  // Many newlines to two
  assertEqual(normalizeBlankLines('a\n\n\n\n\nb'), 'a\n\nb', 'Reduces many newlines to two')

  // Two newlines unchanged
  assertEqual(normalizeBlankLines('a\n\nb'), 'a\n\nb', 'Two newlines unchanged')

  // Single newline unchanged
  assertEqual(normalizeBlankLines('a\nb'), 'a\nb', 'Single newline unchanged')
})
...
testGroup('Line Trimming', () => {
  // Trailing spaces
  assertEqual(trimLines('hello   \nworld  '), 'hello\nworld', 'Removes trailing spaces from lines')

  // Leading spaces
  assertEqual(trimLines('   hello\n  world'), 'hello\nworld', 'Removes leading spaces from lines')
...
testGroup('Markdown Stripping', () => {
  ...
  // Headers
  assertEqual(stripMarkdown('# Header\nContent'), 'Header\nContent', 'Strips header markdown')
  ...
  // Blockquotes
  assertEqual(stripMarkdown('> This is a quote'), 'This is a quote', 'Strips blockquote markdown')
...
testGroup('Main Sanitize Function', () => {
  // Comprehensive test
  const messyText = '\uFEFFHello\u200B world\u00A0\u00A0with   extra\n\n\n\nspaces'
  assertEqual(
    sanitize(messyText),
...
testGroup('Sanitize AI Output', () => {
  ...
  const aiResponse = '**Developed** software using `React` and worked on [various projects](url)'
  assertEqual(
...
testGroup('Object Sanitization', () => {
  ...
  assertEqual(sanitizedMixed.count, 42, 'Preserves numbers in mixed object')
  assertEqual(sanitizedMixed.active, true, 'Preserves booleans in mixed object')
...
testGroup('Resume-Specific Scenarios', () => {
  ...
  const highlight = '\u200BDeveloped **RESTful APIs** using `Node.js` and `Express`\u200B'
  assertEqual(
  ...
  const summary = 'Experienced developer.\n\nPassionate about clean code.'
  assertEqual(
  ...
  const resume = {
    personalInfo: {
      name: '\uFEFFJohn Doe\u200B',
      summary: '**Experienced** developer with [proven track record](url).',
    },
    workExperience: [
      {
        company: 'Tech Corp\u00A0',
  ...
  assertEqual(sanitizedResume.personalInfo.name, 'John Doe', 'Sanitizes name in resume')
  assertEqual(
  ...
  if (firstExperience) {
    assertEqual(firstExperience.company, 'Tech Corp', 'Sanitizes company name')
    assertEqual(firstExperience.title, 'SeniorEngineer', 'Removes zero-width from title')
    assertEqual(
